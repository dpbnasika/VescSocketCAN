##
# A part of the library is taken from https://github.com/tlalexander/rover_control/blob/master/vesc.py 
# and modifed for our own purpose 
##
...
class CanVesc:
    ...
    def set_motor_duty(self, duty, device_id):
        try:
            # send duty command
            duty_cycle = int(duty*100000)
            data = bytearray(duty_cycle.to_bytes(7, byteorder='big'))
            data[0] = 0x01  # "rx_buffer_last_id" in VESC firmware
            data[1] = 0x00  # "commands_send" in VESC firmware
            data[2] = VescCommand.COMM_SET_DUTY.value
            frame = self.build_can_frame(CanCommand.PROCESS_SHORT_BUFFER, device_id, data)
            print(frame) if _DEBUG_FRAME else None
            self.sock.send(frame)
        except socket.error:
...
while True:
    #vesc.set_motor_duty(0.1,15)
    #vesc.set_motor_duty(0.1,16)
    vesc.set_motor_rpm(2816,15)
    vesc.set_motor_rpm(2816,16)
    cf, addr = vesc.sock.recvfrom(32)
    can_id,can_dlc,data = vesc.dissect_can_frame(cf)
    if((can_id - 2147483648)==2319 or (can_id - 2147483648)==2320  ):
        rpm = data[0]<<24 or data[1]<<16 or data[2]<<8 or data[3]
        print (rpm)
        print('Received: can_id=%x, can_dlc=%x, data=%s' % vesc.dissect_can_frame(cf))
